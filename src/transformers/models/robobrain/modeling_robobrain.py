...
from .configuration_robobrain import RoboBrainConfig, RoboBrainTextConfig, RoboBrainVisionConfig
...
class RoboBrainVisionPatchEmbed(nn.Module):
    def __init__(
        self,
        patch_size: int = 14,
...
class RoboBrainVisionRotaryEmbedding(nn.Module):
...
class RoboBrainPatchMerger(nn.Module):
...
class RoboBrainVisionFlashAttention2(nn.Module):
...
class RoboBrainVisionAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int = 16) -> None:
        super().__init__()
        self.num_heads = num_heads
...
class RoboBrainVisionSdpaAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int = 16) -> None:
        super().__init__()
        self.num_heads = num_heads
        self.qkv = nn.Linear(dim, dim * 3, bias=True)
...
ROBOBRAIN_VISION_ATTENTION_CLASSES = {
    "eager": RoboBrainVisionAttention,
    "flash_attention_2": RoboBrainVisionFlashAttention2,
    "sdpa": RoboBrainVisionSdpaAttention,
}


class RoboBrainVisionBlock(nn.Module):
    def __init__(self, config, attn_implementation: str = "sdpa") -> None:
        super().__init__()
        self.norm1 = Qwen2RMSNorm(config.hidden_size, eps=1e-6)
        self.norm2 = Qwen2RMSNorm(config.hidden_size, eps=1e-6)
        self.attn = ROBOBRAIN_VISION_ATTENTION_CLASSES[attn_implementation](
            config.hidden_size, num_heads=config.num_heads
        )
        self.mlp = RoboBrainMLP(config, bias=True)
...
@auto_docstring
class RoboBrainPreTrainedModel(PreTrainedModel):
    config_class = RoboBrainConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["RoboBrainDecoderLayer", "RoboBrainVisionBlock"]
...
class RoboBrainVisionTransformerPretrainedModel(RoboBrainPreTrainedModel):
    config_class = RoboBrainVisionConfig
    _no_split_modules = ["RoboBrainVisionBlock"]
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.blocks = nn.ModuleList(
            [RoboBrainVisionBlock(config, config._attn_implementation) for _ in range(config.depth)]
        )
        self.merger = RoboBrainPatchMerger(
...
@dataclass
class RoboBrainModelOutputWithPast(ModelOutput):
    """
...
class RoboBrainRotaryEmbedding(nn.Module):
    def __init__(self, config: RoboBrainTextConfig, device=None):
        super().__init__()
    ...
    def forward(self, x, position_ids):
        # In contrast to other models, RoboBrain has different position ids for the grids
...
class RoboBrainAttention(nn.Module):
    """
    Multi-headed attention from 'Attention Is All You Need' paper. Modified to use sliding window attention: Longformer
    and "Generating Long Sequences with Sparse Transformers".
    """

    def __init__(self, config: RoboBrainTextConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        self.rotary_emb = RoboBrainRotaryEmbedding(config=config)
...
class RoboBrainFlashAttention2(RoboBrainAttention):
    """
    RoboBrain flash attention module, following RoboBrain attention module. This module inherits from `RoboBrainAttention`
...
class RoboBrainSdpaAttention(RoboBrainAttention):
    """
    ...
    def forward(
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "RoboBrainModel is using RoboBrainSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
ROBOBRAIN_ATTENTION_CLASSES = {
    "eager": RoboBrainAttention,
    "flash_attention_2": RoboBrainFlashAttention2,
    "sdpa": RoboBrainSdpaAttention,
}


class RoboBrainDecoderLayer(nn.Module):
    def __init__(self, config: RoboBrainTextConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.self_attn = ROBOBRAIN_ATTENTION_CLASSES[config._attn_implementation](config, layer_idx)
...
@auto_docstring
class RoboBrainTextModel(RoboBrainPreTrainedModel):
    config_class = RoboBrainTextConfig

    def __init__(self, config: RoboBrainTextConfig):
        ...
        self.layers = nn.ModuleList(
            [RoboBrainDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
            ...
            if attention_mask is not None and past_key_values is not None:
                ...
                if is_padding_right:
                    raise ValueError(
                        ...
                        " this may lead to unexpected behaviour for Flash Attention version of RoboBrain. Make sure to "
...
@auto_docstring
class RoboBrainModel(RoboBrainPreTrainedModel):
    base_model_prefix = ""
    ...
    def __init__(self, config):
        ...
        self.language_model = RoboBrainTextModel._from_config(config.text_config)
...
@dataclass
class RoboBrainCausalLMOutputWithPast(ModelOutput):
    """
    Base class for RoboBrain causal language model (or autoregressive) outputs.
...
class RoboBrainForConditionalGeneration(RoboBrainPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = RoboBrainModel(config)
...
__all__ = ["RoboBrainForConditionalGeneration", "RoboBrainModel", "RoboBrainPreTrainedModel", "RoboBrainTextModel"]
